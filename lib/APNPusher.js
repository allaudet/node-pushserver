...
var onTransmissionError = function (errorCode, notification, recipient) {
    ...
    if (errorCode === apn.Errors.invalidToken && recipient) {
        var token = null;

        if (typeof recipient === 'string') {
            token = recipient.toLowerCase();
            pushAssociations.removeDevice(token);
        } else if (recipient.token) {
            token = recipient.token.toString('hex').toLowerCase();
            pushAssociations.removeDevice(token);
        }
    }
};
...
var onFeedback = function (deviceInfos) {
    ...
    if (deviceInfos.length > 0) {
        pushAssociations.removeDevices(deviceInfos.map(function (deviceInfo) {
            return deviceInfo.device.token.toString('hex').toLowerCase();
        }));
    }
};
